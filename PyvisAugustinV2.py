...
def main(args):
    ...
    # Initialisation
    allAddresses = set([outcome])  # Utiliser un set pour éviter les duplications
    node_layer = {}  # Stocker le cercle de première découverte
    node_layer[outcome] = 0
    
    # Queue pour BFS : liste de (adresse, cercle_de_découverte)
        ...
        for addr in current_level:
            ...
            if outcomes is not None:
                for outcome_addr in outcomes:
                    ...
                    # Si c'est un nouveau nœud, l'ajouter avec le niveau suivant
                    if outcome_addr not in allAddresses:
                        ...
                        print(f"    Nouveau: {outcome_short} -> cercle {cercle + 1}")
        
        print(f"  Total adresses découvertes: {len(allAddresses)}")
        print(f"  Nouveaux pour cercle suivant: {len(next_level)}")
    ...
    # Construire le dictionnaire des prédécesseurs pour le layout
    predecessors = {}
    for income in transac:
    ...
    # Organiser les nœuds par couche selon leur première apparition
    layers_nodes = {}
    ...
    print(f"Répartition par couche:")
    for layer in sorted(layers_nodes.keys()):
        print(f"  Couche {layer}: {len(layers_nodes[layer])} nœuds")
    ...
    for income in transac:
        ...
        if income_short not in added_nodes:
            # Utiliser la couche de première découverte
            ...
            net.add_node(income_short, label=income_short, 
                        title=f"{income}\nCouche: {layer}", **node_opts)
        ...
        for outcome in transac[income]:
            ...
            if outcome_short not in added_nodes:
                ...
                net.add_node(outcome_short, label=outcome_short, 
                            title=f"{outcome}\nCouche: {layer}", **node_opts)
    ...
    output_file = f"pyvis_Augustin_cerlce{nb_cercles}.html"
